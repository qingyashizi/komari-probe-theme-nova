...
export type BackgroundOrientationMode = 'auto' | BackgroundOrientation
...
export function parsePingTaskIds(raw: unknown, max = THREE_NET_PING_TASK_LIMIT): number[] {
  let items: unknown[] = []
...
export function parseThreeNetPingNodeTaskBindings(raw: unknown): Record<string, number[]> {
...
export function parseBackgroundSources(value: unknown): string[] {
  if (typeof value !== 'string')
    return []
...
export function pickSessionBackground(key: string, sources: string[]): string {
  if (!sources.length)
    return ''
...
export function getViewportOrientation(): BackgroundOrientation {
...
export function getOrientationBackgroundKey(mode: 'light' | 'dark', orientation: BackgroundOrientation): string {
  return `${mode}Background${orientation === 'portrait' ? 'Portrait' : 'Landscape'}Url`
}

export function getBaseBackgroundKey(mode: 'light' | 'dark'): string {
