...
import MetricChartHeader from '@/components/charts/MetricChartHeader.vue'
import MetricSeriesChartCard from '@/components/charts/MetricSeriesChartCard.vue'
...
import { getGpuDeviceNames, LOAD_METRIC_KEYS, metricSeriesToRecordFormat, metricValue, PING_METRIC_KEYS, statusToRecordFormat } from '@/utils/loadChartRecords'
...
async function loadMetricHistoryRecords(params: Pick<MetricQueryParams, 'hours' | 'start' | 'end'>): Promise<MetricHistoryData | null> {
  ...
  return {
    records: metricSeriesToRecordFormat(metricHistory.series, props.uuid, nodeInfo.value),
    ...
    <Spinner :show="loading">
      ...
      <div v-else class="gap-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        ...
        <CardX v-if="isChartCardEnabled('gpu')" size="small" class="bg-background/50 border-none hover:bg-background transition-all rounded-md" :style="getChartCardStyle('gpu')">
          <template #header>
            <MetricChartHeader title="GPU 利用率" icon="tabler:device-desktop-analytics" tone="cyan" :subtitle="getGpuDeviceNames(latestStatus, nodeInfo?.gpu_name)">
              <div class="text-xs flex gap-1 items-baseline shrink-0">
